...
import { lopList, lichHocList, danhGiaCanBoList } from '../data/giaoDucTuVan';
...
export default function DanhGiaCanBoDetail() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (!isNew && id) {
      const dg = danhGiaCanBoList.find(dg => dg.id === id);
      if (dg) setData({ ...dg });
    } else {
      setData(initData);
    }
  ...
  const handleSubmit = e => {
    e.preventDefault();
    if (!data.lopId || !data.canBo || !data.hocVien || !data.danhGia || !data.thoiGian) {
  ...
  return (
    <div>
      <h1 style={{ color: '#111', fontSize: 24, fontWeight: 700, marginBottom: 18 }}>{isNew ? 'Thêm đánh giá cán bộ' : 'Chỉnh sửa đánh giá cán bộ'}</h1>
      <form className="hv-grid-form" onSubmit={handleSubmit}>
        <div className="hv-grid">
          ...
          <div className="form-group">
            ...
            <select name="lichHocId" value={data.lichHocId} onChange={handleChange} required>
              <option value="">Chọn buổi học</option>
              {lichHocByLop.map(lh => <option key={lh.id} value={lh.id}>{lh.mon} - {lh.thoiGian.replace('T', ' ')}</option>)}
            </select>
          </div>
          <div className="form-group">
            <label>Cán bộ tư vấn *</label>
            <input name="canBo" value={data.canBo} onChange={handleChange} required placeholder="Tên cán bộ tư vấn" />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <input name="danhGia" value={data.danhGia} onChange={handleChange} required placeholder="Đánh giá" />
          </div>
          <div className="form-group">
            <label>Nhận xét</label>
            <input name="nhanXet" value={data.nhanXet} onChange={handleChange} placeholder="Nhận xét" />
          </div>
          <div className="form-group">
